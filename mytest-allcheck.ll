; ModuleID = 'mytest.bc'
source_filename = "tolerance.c"
target datalayout = "e-m:e-p:32:32-i64:64-v128:64:128-a:0:32-n32-S64"
target triple = "armv7-ellcc-linux"

@.str = private unnamed_addr constant [4 x i8] c"%d\0A\00", align 1

; Function Attrs: nounwind
define arm_aapcscc i32 @main() #0 {
  %allocaVec = alloca <4 x i32>, align 16
  %1 = alloca i32, align 4
  %allocaVec1 = alloca <4 x i32>, align 16
  %2 = alloca i32, align 4
  %allocaVec2 = alloca <4 x i32>, align 16
  %3 = alloca i32, align 4
  store i32 3, i32* %1, align 4
  store i32 4, i32* %2, align 4
  store i32 5, i32* %3, align 4
  %4 = load i32, i32* %1, align 4
  %insertElmt = insertelement <4 x i32> undef, i32 %4, i32 0
  %insertElmt3 = insertelement <4 x i32> %insertElmt, i32 %4, i32 1
  %insertElmt4 = insertelement <4 x i32> %insertElmt3, i32 %4, i32 2
  %insertElmt5 = insertelement <4 x i32> %insertElmt4, i32 %4, i32 3
  store <4 x i32> %insertElmt5, <4 x i32>* %allocaVec, align 16
  %5 = load i32, i32* %2, align 4
  %insertElmt6 = insertelement <4 x i32> undef, i32 %5, i32 0
  %insertElmt7 = insertelement <4 x i32> %insertElmt6, i32 %5, i32 1
  %insertElmt8 = insertelement <4 x i32> %insertElmt7, i32 %5, i32 2
  %insertElmt9 = insertelement <4 x i32> %insertElmt8, i32 %5, i32 3
  store <4 x i32> %insertElmt9, <4 x i32>* %allocaVec1, align 16
  %6 = load <4 x i32>, <4 x i32>* %allocaVec, align 16
  %7 = load <4 x i32>, <4 x i32>* %allocaVec1, align 16
  %Vop = add <4 x i32> %6, %7
  store <4 x i32> %Vop, <4 x i32>* %allocaVec
  %8 = load <4 x i32>, <4 x i32>* %allocaVec, align 4
  %extractE = extractelement <4 x i32> %8, i64 0
  %extractE10 = extractelement <4 x i32> %8, i64 1
  %extractE11 = extractelement <4 x i32> %8, i64 2
  %Recovery = alloca i32, align 4
  %sum = add i32 %extractE, %extractE10
  %sum12 = add i32 %sum, %extractE11
  %9 = add nsw i32 %4, %5
  %Fmul = mul i32 %9, 3
  store i32 %9, i32* %Recovery
  %Fcmp = icmp ne i32 %sum12, %Fmul
  br i1 %Fcmp, label %10, label %14

; <label>:10:                                     ; preds = %0
  %remThree = sdiv i32 %sum12, %extractE
  %FcmpThree = icmp ne i32 %remThree, 3
  br i1 %FcmpThree, label %11, label %12

; <label>:11:                                     ; preds = %10
  store i32 %extractE, i32* %Recovery, align 4
  br label %13

; <label>:12:                                     ; preds = %10
  store i32 %9, i32* %Recovery, align 4
  br label %13

; <label>:13:                                     ; preds = %12, %11
  br label %14

; <label>:14:                                     ; preds = %0, %13
  %15 = load i32, i32* %Recovery, align 4
  store i32 %15, i32* %1, align 4
  %16 = load i32, i32* %3, align 4
  %insertElmt13 = insertelement <4 x i32> undef, i32 %16, i32 0
  %insertElmt14 = insertelement <4 x i32> %insertElmt13, i32 %16, i32 1
  %insertElmt15 = insertelement <4 x i32> %insertElmt14, i32 %16, i32 2
  %insertElmt16 = insertelement <4 x i32> %insertElmt15, i32 %16, i32 3
  store <4 x i32> %insertElmt16, <4 x i32>* %allocaVec2, align 16
  %17 = load i32, i32* %1, align 4
  %insertElmt17 = insertelement <4 x i32> undef, i32 %17, i32 0
  %insertElmt18 = insertelement <4 x i32> %insertElmt17, i32 %17, i32 1
  %insertElmt19 = insertelement <4 x i32> %insertElmt18, i32 %17, i32 2
  %insertElmt20 = insertelement <4 x i32> %insertElmt19, i32 %17, i32 3
  store <4 x i32> %insertElmt20, <4 x i32>* %allocaVec, align 16
  %18 = load <4 x i32>, <4 x i32>* %allocaVec2, align 16
  %19 = load <4 x i32>, <4 x i32>* %allocaVec, align 16
  %Vop21 = add <4 x i32> %18, %19
  store <4 x i32> %Vop21, <4 x i32>* %allocaVec2
  %20 = load <4 x i32>, <4 x i32>* %allocaVec2, align 4
  %extractE23 = extractelement <4 x i32> %20, i64 0
  %extractE24 = extractelement <4 x i32> %20, i64 1
  %extractE25 = extractelement <4 x i32> %20, i64 2
  %Recovery26 = alloca i32, align 4
  %sum27 = add i32 %extractE23, %extractE24
  %sum28 = add i32 %sum27, %extractE25
  %21 = add nsw i32 %16, %17
  %Fmul22 = mul i32 %21, 3
  store i32 %21, i32* %Recovery26
  %Fcmp29 = icmp ne i32 %sum28, %Fmul22
  br i1 %Fcmp29, label %22, label %26

; <label>:22:                                     ; preds = %14
  %remThree30 = sdiv i32 %sum28, %extractE23
  %FcmpThree31 = icmp ne i32 %remThree30, 3
  br i1 %FcmpThree31, label %23, label %24

; <label>:23:                                     ; preds = %22
  store i32 %extractE23, i32* %Recovery26, align 4
  br label %25

; <label>:24:                                     ; preds = %22
  store i32 %21, i32* %Recovery26, align 4
  br label %25

; <label>:25:                                     ; preds = %24, %23
  br label %26

; <label>:26:                                     ; preds = %14, %25
  %27 = load i32, i32* %Recovery26, align 4
  store i32 %27, i32* %3, align 4
  %28 = load i32, i32* %1, align 4
  %29 = call arm_aapcscc i32 (i8*, ...) @printf(i8* getelementptr inbounds ([4 x i8], [4 x i8]* @.str, i32 0, i32 0), i32 %28)
  ret i32 0
}

declare arm_aapcscc i32 @printf(i8*, ...) #1

attributes #0 = { nounwind "correctly-rounded-divide-sqrt-fp-math"="false" "disable-tail-calls"="false" "less-precise-fpmad"="false" "no-frame-pointer-elim"="true" "no-frame-pointer-elim-non-leaf" "no-infs-fp-math"="false" "no-jump-tables"="false" "no-nans-fp-math"="false" "no-signed-zeros-fp-math"="false" "no-trapping-math"="false" "stack-protector-buffer-size"="8" "target-cpu"="cortex-a8" "target-features"="+dsp,+neon,+vfp3,-crypto,-d16,-fp-armv8,-fp-only-sp,-fp16,-vfp4" "unsafe-fp-math"="false" "use-soft-float"="false" }
attributes #1 = { "correctly-rounded-divide-sqrt-fp-math"="false" "disable-tail-calls"="false" "less-precise-fpmad"="false" "no-frame-pointer-elim"="true" "no-frame-pointer-elim-non-leaf" "no-infs-fp-math"="false" "no-nans-fp-math"="false" "no-signed-zeros-fp-math"="false" "no-trapping-math"="false" "stack-protector-buffer-size"="8" "target-cpu"="cortex-a8" "target-features"="+dsp,+neon,+vfp3,-crypto,-d16,-fp-armv8,-fp-only-sp,-fp16,-vfp4" "unsafe-fp-math"="false" "use-soft-float"="false" }

!llvm.module.flags = !{!0, !1}
!llvm.ident = !{!2}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 1, !"min_enum_size", i32 4}
!2 = !{!"clang version 4.0.0 (trunk)"}
